        ...
        uint32_t *assoc_id)
{
    ...
    if (resp->sctp_state != SCTP_STATE_ESTABLISHED) {
        if (ric_agent_info[instance] != NULL) {
            ...
            RIC_AGENT_ERROR("invalid nb/instance %lu in sctp_new_association_resp\n", instance);
            return -1;
        }
        return 0;
    }
    ...
    */
    ...
    RIC_AGENT_INFO("new sctp assoc resp %d for nb %lu\n", resp->assoc_id, instance);

    ric = ric_agent_get_info(instance, -1);
    if (ric == NULL) {
        RIC_AGENT_ERROR("[%s]: ric agent info not found %lu\n", __func__, instance);
        return -1;
    }
        ...
        uint32_t *assoc_id)
{
    ...
    if (ric == NULL) {
        RIC_AGENT_ERROR("ric_agent_handle_sctp_data_ind: ric agent info not found %lu\n", instance);
        return;
    }

    RIC_AGENT_DEBUG("sctp_data_ind instance %lu assoc %d", instance, ind->assoc_id);
        ...
        uint32_t *assoc_id)
{
    ...
    if (timer_id == ric->ric_connect_timer_id) {
        ...
        *assoc_id = ric->data_conn_assoc_id;
        ...
        RIC_AGENT_INFO("invalid timer expiry instance %lu timer_id %ld", instance, timer_id);
    }
    DevAssert(ret == 0);
}
...
void *ric_agent_task(void *args)
{
    ...
    while (1) {
        ...
        if (outlen) {
            ...
            AssertFatal(ric != NULL, "ric agent info not found %lu\n", instance);
...
// These marcos will be populated by external parameters
...
#pragma GCC diagnostic pop
	     ...
	     strcmp(*ric_params[RIC_CONFIG_IDX_ENABLED].strptr, "yes") == 0)
        {
            ...
            // LTE
                ...
                // init e2 conf
                // in eNB, F1AP no longer exist in the latest OAI
                e2_conf[i]->enabled = 1;
                e2_conf[i]->node_name = ENB_CONFIG_STRING_ENB_LIST; // openair2/ENB_APP/RRC_paramsvalues.h
